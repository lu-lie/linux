				  ...
				  u64 flags)
{
	...
	/* If the directory already exists, debugfs_create_dir will fail. It
	 * could have been created by map_update_elem before, or another system
	 * that uses debugfs.
	 *
	 * Note that the directory name passed as value should not be longer
	 * than map->value_size, including the '\0' at the end.
	 */
...
__bpf_kfunc int bpf_relay_output(struct bpf_map *map, void *data,
				 u64 data__sz, u32 flags)
{
	struct bpf_relay_map *rmap;
